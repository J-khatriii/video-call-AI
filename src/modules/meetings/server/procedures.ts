...
import { meetingsInsertSchema, meetingsUpdateShema } from "../schemas";
        ...
        const agentSpeakers = await db
            ...
            .then((agents) => 
                agents.map((agent) => ({
                    ...agent,
                    image:
                    generatedAvatarUrl({ seed: agent.name, variant: "botttsNeutral"}),
                }))   
            )
        ...
        const transcriptWithSpeakers = transcript.map((item) => {
            ...
            if(!speaker) {
                return {
                    ...
                    user: {
                        name: "Unknown",
                        image: generatedAvatarUrl({ seed: "Unknown", variant: "initials"}),
                    },
                }
            }
            ...
            await streamVideo.upsertUsers([
                {
                    ...
                    role: "user",
                    image: generatedAvatarUrl({ seed: exitingAgent.name, variant: "botttsNeutral"})
                }
            ])
