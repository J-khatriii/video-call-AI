        ...
        <div className="bg-white rounded-lg border px-4 py-5 flex flex-col gap-y-4 w-full">
            ...
            <div className="relative">
                <Input
                    ...
                    value={searchQuery}
                    onChange={(e) => seySearchQuery(e.target.value)}
                />
                            ...
                            <div key={item.start_ts} className="flex flex-col gap-y-2 hover:bg-muted p-4 rounded-md border">
                                <div className="flex gap-x-2 items-center">
                                    <Avatar className="size-6">
                                        <AvatarImage src={item.user.image ?? generatedAvatarUrl({ seed: item.user.name, variant:"initials"})} alt="User Avatar" />
                                    </Avatar>
                                    <p className="text-sm font-medium">{item.user.name}</p>
                                    <p className="text-sm text-blue-500 font-medium">
                                        {
                                            format(
                                                new Date(0,0,0,0,0,0, item.start_ts), "mm:ss"
                                            )
                                        }
                                    </p>
                                </div>
